    ...
    class Program
    {
        static void Main()
        {
            ...
            while (incorrect)
            {
                ...
                {
                    Console.WriteLine("Enter item ID to remove that item from your cart.\n Your cart:");
                    List<PcComponent> RemoveCart = Cart.OrderBy(o => o.ID).ToList();
                    foreach (var PcComponent in RemoveCart)
                    {
                        Console.WriteLine(" ID = {0}, Type = {1}, Name = {2}, Manufacturer = {3}, Price = {4}", PcComponent.ID, PcComponent.Type, PcComponent.Name, PcComponent.Manufacturer, PcComponent.Price);
                    }
                    string inputRemove = Console.ReadLine();
                    if (int.TryParse(inputRemove, out userremove))
                    {
                        if (Cart.Exists(x => x.ID == userremove))
                        {
                            Cart.RemoveAll(r => r.ID == userremove);
                            Console.Clear();
                            Console.WriteLine("Item with ID = {0} has been removed from your cart.", userremove);

                        }
                        else
                        {
                            Console.Clear();
                            Console.WriteLine("{0} does not match any item ID.", userremove);
                ...
                else if (userinput == "CART")
                {
                    ...
                    List<PcComponent> SortedCart = Cart.OrderBy(o => o.ID).ToList();
                    foreach (var PcComponent in SortedCart)
                    {
